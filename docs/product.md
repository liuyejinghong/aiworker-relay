...
2. 用户明确选择 profile；系统只检查该 profile 是否可用，并使用其默认推理策略，不静默替换模型或降低固定档位。
3. profile 为冻结时，拒绝派发并报告冻结原因。
4. v0.1 不接受 run 级推理档位；固定策略 profile 使用其用户确认的默认档位，“自动”策略 profile 不固定具体档位。
