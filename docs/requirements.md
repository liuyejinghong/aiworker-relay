...
- 首发外部 harness 只有隔离的 `codex exec --json --ephemeral --strict-config --approve-for-me --output-last-message`。每个 run 使用独立 `CODEX_HOME`、隔离 HOME 与不继承 `:workspace` 的 `aiworker` permission profile；文件系统默认拒绝，只读放行最小运行时、实际工具链与 linked-worktree Git metadata，只写 detached worktree 和 run-scoped 隔离 HOME，工具网络关闭，source checkout 与真实 HOME 不可读写，source index 另行显式拒绝，worktree `.env*` 禁读。项目配置按 untrusted 处理，安全 shell-policy 与用户明确选择的 reasoning 由 CLI 固定。不得同时传旧 `--sandbox`，不得使用危险的 approvals/sandbox bypass，也不建设备用 CLI 或模型 SDK 路径。
