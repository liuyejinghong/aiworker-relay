...
- v0.1.x 公开预览只支持 macOS arm64/x86_64 与标准 CPython 3.12、3.13、3.14。Windows、Linux、free-threaded / 其他 Python build、其他 Python minor 的代码路径不构成发布支持声明，必须在各自安装、Keyring、sandbox、dependency lock 与停止证据完成后另行决定。
...
- 首发外部 harness 只有隔离的 `codex exec --json --ephemeral --strict-config --approve-for-me --output-last-message`。每个 run 使用独立 `CODEX_HOME`、隔离 HOME 与不继承 `:workspace` 的 `aiworker` permission profile；文件系统默认拒绝，只读放行 Codex 最小平台运行时、实际工具链与 linked-worktree Git metadata，并放行 detached worktree 和 run-scoped 隔离 HOME 写入。工具网络关闭，source index 与 worktree `.env*` 显式拒绝，普通 source checkout 与真实 HOME 路径必须通过 live sentinel 拒绝。项目配置按 untrusted 处理，安全 shell-policy 与用户明确选择的 reasoning 由 CLI 固定。当前 macOS Codex 普通 shell 仍可访问 host `/tmp` / `/var/tmp`，不得把当前边界描述为完整 host isolation。不得同时传旧 `--sandbox`，不得使用危险的 approvals/sandbox bypass，也不建设备用 CLI 或模型 SDK 路径。
...
本次已完成：符合 Codex marketplace source 约定的 AIworker Relay Plugin package、`aiworker-relay` Skill、应用级 bootstrap launcher、固定 loopback `external-workersd`、Profile / Key / run API、账户与公开跑分的按需读取、静态多页 Web、隔离 worktree / `codex exec` 路径、JSONL 证据与两阶段停止代码；canonical package `VERSION`、version + source fingerprint 的 setup-only runtime 收敛、活跃 run 更新延后、持久控制面迁移、更新失败恢复与 dispatch 身份不一致拒绝；三个 macOS/Python-minor hash locks、固定构建环境、package-set readback 与 dependency identity；以及指向 reviewed v0.1.19 commit `89d8d564c80cd4de59d7908a6a7114f4c2d03f54` 的 immutable catalog。该 release source 已通过隔离 Codex CLI 安装、真实用户 Codex Desktop 共享安装态的 `v0.1.19 → v0.1.18 → v0.1.19` 验收，以及最终 Plugins Directory 人工回读；GitHub ruleset `21734294` 已对 `main` active 生效，v0.1.19 annotated tag 与 [GitHub developer pre-release](https://github.com/liuyejinghong/aiworker-relay/releases/tag/v0.1.19) 已发布。
...
- 当前 macOS Codex 普通 shell 仍可访问 host `/tmp` / `/var/tmp`。公开预发布是否接受这一明确限制，还是等待上游 runtime 提供可验证的完整 deny？
