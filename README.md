...
**Developer pre-release.** [v0.1.19](https://github.com/liuyejinghong/aiworker-relay/releases/tag/v0.1.19) is published from an annotated tag on reviewed Plugin source `89d8d564c80cd4de59d7908a6a7114f4c2d03f54`; the release catalog pins the Git-backed Plugin entry to that exact source rather than mutable `main`. The real-user Codex Desktop state has completed an idle `v0.1.19 install → exact v0.1.18 rollback → v0.1.19 restore` acceptance: the final Plugin is installed and enabled, bundle/runtime/daemon all report v0.1.19 with source fingerprint `sha256:1acdaffa2c68a43379192a08b0bb19e776dd22d5850b641c09c6fda426bfb20b`, and the saved Key and Profiles were preserved. The earlier v0.1.18 NVIDIA write / TERM evidence remains the external-run acceptance; no task was dispatched during the update round trip. Current macOS Codex runtimes still expose host temp directories to ordinary sandboxed shell processes, so this release does not claim complete host isolation. The NVIDIA Profile remains `unverified` pending an accepted promotion contract.

Source repository: [liuyejinghong/aiworker-relay](https://github.com/liuyejinghong/aiworker-relay). The documented Git marketplace flow and its shared Codex Desktop install state have been observed through install, rollback and restore; this does not claim a separate native update-button behavior or a stable release.
