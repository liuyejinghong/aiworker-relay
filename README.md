...
**Pre-release.** v0.1.19 is the current source candidate at reviewed Git commit `89d8d564c80cd4de59d7908a6a7114f4c2d03f54`; v0.1.18 remains the real-user installed candidate that completed the managed NVIDIA acceptance run. The release-catalog candidate pins the Git-backed Plugin entry to that exact v0.1.19 source rather than mutable `main`. v0.1.19 adds deterministic source-fingerprint convergence plus per-Python-minor, hash-checked macOS runtime locks; the candidate records the accepted lock and complete resolved package set in its runtime identity. Its exact-SHA Plugin has passed an isolated Codex CLI installation, but it has not replaced the real-user Plugin/runtime or been released. Current macOS Codex runtimes still expose host temp directories to ordinary sandboxed shell processes, so this candidate does not claim complete host isolation. The NVIDIA Profile remains `unverified` pending an accepted promotion contract. No tag or GitHub Release has been created.
